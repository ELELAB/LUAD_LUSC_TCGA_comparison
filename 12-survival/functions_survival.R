...
cox_regression <- function(clinical,dataFilt,gene_name, threshDown, threshUp,method,final_table,num){
  ...
  cox.model <- coxph(Surv(time,status)~group+tumor_stage+age+gender,data=clinical)
    ...
    sum_cox <- sum_cox[,-c(2,3,4)]
    colnames(sum_cox) <- c("coef","p-value")
    ...
    names(vector) <- c("coef(group_low)","p-value(group_low)","fdr(group_low)",
                       "coef(stageii)","p-value(stageii)","fdr(stageii)",
                       "coef(stageiii)","p-value(stageiii)","fdr(stageiii)",
                       "coef(stageiv)","p-value(stageiv)","fdr(stageiv)",
                       "coef(age>65)","p-value(age>65)","fdr(age>65)",
                       "coef(gendermale)","p-value(gendermale)","fdr(gendermale)")
