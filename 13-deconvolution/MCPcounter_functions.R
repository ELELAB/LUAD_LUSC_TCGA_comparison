...
# function created by Marta
# label the samples according to the immune-cell abundance

abundanceRange <- function(estimation,cellType){
  s <- summary(estimation[cellType,])
  v <- c()
  for (i in estimation[cellType,]) {
    if (i >= min(s) & i< s[2]) {
      v <- c(v,1)
    }
    if (i>= s[2] & i< s[3]) {
...
#---------------------------------------------------------------
# edgeR with immune-cell correction
#----------------------------------------------------------------

edgeR_tss_immune <- function(dataframe,edgeR_name,up_name,down_name,Tcells){
  ...
  # design matrix
  design.mat <- model.matrix(~condition+tss+Tcells, data=y$samples)
